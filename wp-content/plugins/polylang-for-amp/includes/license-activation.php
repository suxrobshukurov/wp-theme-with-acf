...
add_action( 'wp_ajax_ampforwp_get_licence_activate_update', 'ampforwp_polylang_get_extension_licence_activate_update' );

function ampforwp_polylang_get_extension_licence_activate_update(){
        ...
        if( isset($selectedOption['amp-license']) && "" != $selectedOption['amp-license']){
            // data to send in our API request
                ...
                if ( ! empty( $message ) ) {
                    ...
                    $limit = ampforwp_polylang_set_extension_plugin_limit( true, $license_data, $ampforwp_license_activate);
...
function ampforwp_polylang_extension_deactivate_license() {
    if(!wp_verify_nonce( $_REQUEST['verify_nonce'], 'verify_extension' ) ) {
        echo json_encode(array("status"=>300,"message"=>'Request not valid'));
        die;
    }
...
add_action( 'wp_ajax_ampforwp_deactivate_license', 'ampforwp_polylang_extension_deactivate_license' );


function ampforwp_polylang_set_extension_plugin_limit( $force=false, $license_data='', $data) {
