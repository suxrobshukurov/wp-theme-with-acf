...
class WPSEO_Post_Type_Sitemap_Provider implements WPSEO_Sitemap_Provider {
	...
	protected function get_post_type_count( $post_type ) {
		...
		/**
		 * Filter JOIN query parts for type count of post type.
		 *
		 * @param string $join      SQL parts, defaults to empty string.
		 * @param string $post_type Post type name.
		 */
		$join_filter = apply_filters( 'wpseo_typecount_join', '', $post_type );

		/**
		 * Filter WHERE query parts for type count of post type.
		 *
		 * @param string $where     SQL parts, defaults to empty string.
		 * @param string $post_type Post type name.
		 */
	...
	protected function get_posts( $post_type, $count, $offset ) {
		...
		if ( ! isset( $filters[ $post_type ] ) ) {
			...
			$filters[ $post_type ] = [
				/**
				 * Filter JOIN query parts for the post type.
				 *
				 * @param string $join      SQL parts, defaults to false.
				 * @param string $post_type Post type name.
				 */
				'join'  => apply_filters( 'wpseo_posts_join', false, $post_type ),

				/**
				 * Filter WHERE query parts for the post type.
				 *
				 * @param string $where     SQL parts, defaults to false.
				 * @param string $post_type Post type name.
				 */
